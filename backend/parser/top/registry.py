"""
top 格式版本注册表（类似 iostat/registry 和 ps/registry）。
...
def _banner_match(file_banner: str, known_banner: str) -> bool:
    ...
    例：
      known = "Linux WQWbb v7.3.3"     → file_banner 必须完全相等
      known = "Linux OSWbb v*"        → file_banner 只要以 "Linux OSWbb v" 开头即可
    """
    if not known_banner:
...
def _fingerprints_match(file_fp: dict, known_fp: dict) -> bool:
    """fingerprint 匹配：banner（支持 glob）+ cycle_pattern + top_header 集合全部相等"""
    if not _banner_match(file_fp.get('banner') or '', known_fp.get('banner') or ''):
        return False
    if file_fp.get('cycle_pattern') != known_fp.get('cycle_pattern'):
        return False
    file_hdr = set(file_fp.get('top_header') or [])
    known_hdr = set(known_fp.get('top_header') or [])
    if file_hdr != known_hdr:
        return False
    return True


class TopVersionRegistry:
    """top 格式版本注册表，负责 detect + dispatch + 未知格式归档。"""
    ...
    def _load_all(self) -> None:
        ...
        for entry in sorted(self.versions_dir.iterdir(), reverse=True):
            ...
            mod = importlib.import_module(
                f'backend.parser.top.versions.{entry.name}.parser'
            )
    ...
    def detect(self, filepath: str) -> str:
        ...
        匹配规则（全部满足）：
          ...
          3. top_header 集合相等（顺序无关）

        任一候选都不匹配 → 抛 UnknownTopFormat + 归档样本。
        """
        ...
        raise UnknownTopFormat(
            banner=file_fp.get('banner'),
            top_header=file_fp.get('top_header'),
    ...
    def _archive_pending(self, filepath: str, file_fp: dict) -> Path:
        ...
        pending 目录：项目根 /iostat-version/pending/（与 ps 共用，historical path）
        """
        import shutil
