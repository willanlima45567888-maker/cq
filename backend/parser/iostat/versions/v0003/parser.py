"""
V0003Parser — WQWbb 新格式（v0002 镜像版）

格式定义从 fingerprint.json 读取（device_header / cpu_header / column_mapping），
本文件只保留解析算法（状态机）。

注意：v0003 当前与 v0002 fingerprint 完全相同（banner / 列名 / cpu header 一致）。
这是为了验证版本注册流程；后续采集到真实新格式时替换 fingerprint.json 即可，
无需修改本解析器。
"""
...
class V0003Parser(BaseParser):
    NAME = 'iostat-v0003'
    VERSION = 'v0003'

    CYCLE_PATTERN = re.compile(r'zzz\s+\*\*\*(.+)')
    CPU_HEADER_PATTERN = re.compile(r'^avg-cpu:')
    # 关键差异：v0003 与 v0002 相同，Device header 无冒号
    DEVICE_HEADER_PATTERN = re.compile(r'^Device\b')
